...
inline Eigen::Matrix3d lidarToCameraRotation(double yaw_rad,
                                             double roll_rad,
                                             double pitch_rad) {
  return (Eigen::AngleAxisd(yaw_rad, Eigen::Vector3d::UnitY()) *
          Eigen::AngleAxisd(pitch_rad, Eigen::Vector3d::UnitX()) *
          Eigen::AngleAxisd(roll_rad, Eigen::Vector3d::UnitZ()))
      .toRotationMatrix();
}

inline Eigen::Matrix3d lidarToCameraRotationFromParams(
    const Vector6d &extrinsic_params) {
  return lidarToCameraRotation(extrinsic_params[0], extrinsic_params[1],
                               extrinsic_params[2]);
}

inline Eigen::Vector3d lidarToCameraEulerDegrees(const Eigen::Matrix3d &R,
                                                 double ref_yaw_deg,
                                                 double ref_roll_deg,
                                                 double ref_pitch_deg) {
  double sin_pitch = std::max(-1.0, std::min(1.0, -R(1, 2)));
  double pitch = std::asin(sin_pitch);
  double cos_pitch = std::cos(pitch);

  double yaw = degToRad(ref_yaw_deg);
  double roll = degToRad(ref_roll_deg);
  if (std::abs(cos_pitch) > 1e-6) {
    yaw = std::atan2(R(0, 2), R(2, 2));
    roll = std::atan2(R(1, 0), R(1, 1));
  }

  Eigen::Vector3d euler_deg(radToDeg(yaw), radToDeg(roll), radToDeg(pitch));
  if (std::abs(euler_deg[0] - ref_yaw_deg) > 180.0) {
    euler_deg[0] += (euler_deg[0] < ref_yaw_deg) ? 360.0 : -360.0;
  }
  if (std::abs(euler_deg[1] - ref_roll_deg) > 180.0) {
    euler_deg[1] += (euler_deg[1] < ref_roll_deg) ? 360.0 : -360.0;
  }
  if (std::abs(euler_deg[2] - ref_pitch_deg) > 180.0) {
    euler_deg[2] += (euler_deg[2] < ref_pitch_deg) ? 360.0 : -360.0;
  }
  return euler_deg;
}
//存放每一个点的索引值和其对应的曲率
